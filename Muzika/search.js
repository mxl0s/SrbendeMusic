...
module.exports = {
  name: "search",
  description: "Traži i izaberi pesmu da pustiš",
  aliases: ["find"],
  cooldown: 3,
  edesc: `Kucaj ovu komandu da vidiš prvih 5 rezultata za tvoju pesmu!\nKomanda: ${PREFIX}search <TITLE | URL>`,
...
async execute(message,args,client) {
    ...
    if (!args.length)
      return attentionembed(message,`Usage: ${message.client.prefix}${module.exports.name} <Ime Klipa>`)
    ...
    if (!message.member.voice.channel)
      return attentionembed(message,"Uđi prvo u voice kanal!")
    ...
    try {
      ...
      if(response === "error"){
        //send error message
        attentionembed(message,"Molim te iskoristi tačan emoji");
