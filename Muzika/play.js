...
module.exports = {
  name: "play",
  aliases: ["p"],
  description: "Pušta pesme iz Youtube-a ili Streama",
  cooldown: 1.5,
  edesc: `Kucaj ovu komandu da pustiš muziku.\nKomanda: ${PREFIX}play <TITLE | URL>`,
...
async execute(message, args, client) {
    ...
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return attentionembed(message, `Moraš biti u istom kanalu kao i ja`);
    //If no args return
    if (!args.length)
      return attentionembed(message, `Komanda: ${message.client.prefix}play <YouTube URL | Ime videa | Soundcloud URL>`);
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#3371FF")
            .setDescription(`**💢 Tražim 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#3371FF")
            .setDescription(`**💢 Tražim 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
        ...
        //send join message
        message.channel.send(new MessageEmbed().setColor("#3371FF")
          .setDescription(`**👍 Ušao u \`${channel.name}\` 📄 zahtevom iz \`#${message.channel.name}\`**`)
    ...
    try {
      ...
      return attentionembed(message, `Ne mogu ući u kanal: ${error}`);
    }
  }
};
