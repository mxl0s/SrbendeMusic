...
module.exports = {
  name: "filter",
  description: "Audio efekti",
  aliases: ["efekat"],
  cooldown: 15,
  edesc: `Kucaj komandu da promeniš efekat trenutne pesme \nKomanda: ${PREFIX}efekat <TipEfekta>`,
...
async execute(message, args, client) {
    ...
    if (message.channel.activeCollector)
      return attentionembed(message, "Već ima aktivna pretraga!");
    //if the user is not in a voice channel return error
    if (!message.member.voice.channel)
      return attentionembed(message, "Uđi u voice kanal prvo")
    ...
    switch (args[0]) {
      ...
      default:
        ...
        .setColor("#3371FF")
        .setTitle("Pogrešan filter, koristi jedan od ovih:")
        .setDescription(`
        \`bassboost\`
        \`8D\`
        \`vaporwave\`
        ...
        \`pulsator\`
        \`subboost\`
        \`clear\`   ---  briše sve filtere`)
        .setFooter(`Naprimer: ${PREFIX}filter bassboost`)
        )
        break;
    }
    //set choice to zero
