...
module.exports = {
  name: "lyrics",
  aliases: ["ly", "tekst"],
  description: "Dobij tekst od pesme koja se trenutno pušta",
  cooldown: 3,
  edesc: `Kucaj komandu dok slušaš pesmu da bi dobio tekst\nUsage: ${PREFIX}lyrics`,
...
async execute(message) {
    ...
    try {
      ...
      if (!lyrics) lyrics = `Nema pronađenog teksta za ${queue.songs[0].title}.`;
    }
    //catch any error
    catch (error) {
      lyrics = `Nema pronađenog teksta za ${queue.songs[0].title}.`;
    }
    //define lyrics Embed
